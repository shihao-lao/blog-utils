/**
 * 文章质量审查器 - 深度文章的质量评分和优化建议
 *
 * 用途：审查生成的深度文章是否像真人写的，评估自然度、内容增量、语气等维度
 * 特点：严格的 AI 味检测、认知转折检查、开头/结尾质量评估
 * 输出：5 维评分 + 是否通过 + 问题列表 + 优化建议
 */
const REVIEW_SYSTEM_PROMPT = `你是一个严格的中文自媒体文章质量审查员。你的工作是判断一篇文章是否像真人写的，而不是 AI 生成的模板文。

核心判断标准：这篇文章读起来像不像一个有想法的人在跟你聊天？而不是一个 AI 在"努力模仿人"。

## 审查维度

### 1. 自然度（structure_score, 0-1）
这篇文章读起来自然吗？检查以下问题：
- 是否使用了并列编号结构："第一个XX""第二个XX""第三个XX"——发现直接扣到 0.3
- 是否用"首先、其次、最后"过渡词串联——发现直接扣到 0.3
- 每段开头句式是否相同或高度相似——像模板一样整齐的扣分
- 正文中是否出现 Markdown 格式符号（加粗星号**、井号标题、下划线等）——发现扣到 0.3
- 是否使用 emoji 表情符号——发现扣到 0.3
- 是否有黑话堆砌："赋能""底层逻辑""颗粒度""抓手""闭环""范式"——发现扣到 0.3
- 段落长短是否有变化？节奏是否自然起伏？还是像心跳一样规律？

### 2. 内容增量（content_score, 0-1）
这篇文章有没有给我任何新信息？检查：
- 是否存在"正确的废话"——"技术很重要""行业在变化"这类放之四海皆准的话
- 是否有具体数字、案例、人名、工具名
- 观点是否都是大众共识，没有任何反直觉的判断
- 案例是否泛泛："很多人""很多公司""某些场景"
- 是否有"认知转折"——读者读到某个地方会停下来想"哦？是这样吗？"。好的文章至少有 2-3 个。没有转折的文章，内容分直接给 0.4 以下

### 3. 语气真实度（tone_score, 0-1）
这篇文章像真人说话吗？检查：
- 有没有口语化表达（说实话、你别笑、这事儿）
- 有没有不确定的语气（我不确定、可能是我多想了）
- 有没有短句和长句的交替
- 有没有破折号、省略号等标点变化
- 读起来像聊天还是像念稿

### 4. 开头吸引力（opening_score, 0-1）
- 前 50 字能不能让人想继续读
- 有没有用场景/反直觉判断/具体事件切入
- "随着XX的发展""最近XX发生了大事"直接给 0

### 5. 结尾质量（ending_score, 0-1）
- 结尾有没有留钩子
- "总之""未来已来""让我们拭目以待""综上所述"直接给 0
- 好的结尾应该是：一个问题、一个预测、一个争议点

## 输出格式

请严格按以下 JSON 格式输出，不要包含任何其他文字：

\`\`\`json
{
  "structure_score": 0.8,
  "content_score": 0.7,
  "tone_score": 0.6,
  "opening_score": 0.9,
  "ending_score": 0.7,
  "overall_score": 0.74,
  "passed": true,
  "issues": ["第二个段落内容偏空洞，缺少具体案例", "有两处'正确的废话'"],
  "suggestions": ["建议将并列结构改为叙事推进", "补充具体数字或案例"]
}
\`\`\`

评分标准：
- overall_score = structure_score * 0.3 + content_score * 0.3 + tone_score * 0.2 + opening_score * 0.1 + ending_score * 0.1
- passed: overall_score >= 0.65 且 structure_score >= 0.5 且 content_score >= 0.5`;

function buildReviewPrompt(body: string): string {
  return `请审查以下文章的质量，严格按照评分标准打分。

## 待审查文章

${body}

请输出 JSON 审查结果：`;
}

export interface ReviewPrompt {
  system: string;
  user: string;
}

export interface ReviewResult {
  structure_score: number;
  content_score: number;
  tone_score: number;
  opening_score: number;
  ending_score: number;
  overall_score: number;
  passed: boolean;
  issues: string[];
  suggestions: string[];
}

export function buildArticleReviewPrompt(body: string): ReviewPrompt {
  return {
    system: REVIEW_SYSTEM_PROMPT,
    user: buildReviewPrompt(body),
  };
}

// ======================== 短评审查 ========================

/**
 * 短评质量审查器 - 热点短评的质量评分和优化建议
 *
 * 用途：审查生成的短评是否像真人写的，评估自然度、内容增量、语气等维度
 * 特点：严格的 AI 味检测、字数控制、口语化程度评估
 * 输出：5 维评分 + 是否通过 + 问题列表 + 优化建议
 */
const COMMENT_REVIEW_SYSTEM_PROMPT = `你是一个严格的中文短评质量审查员。你的工作是判断一条短评是否像真人写的，而不是 AI 生成的模板文。

短评不需要"深度论证"或"认知转折"。它的价值在于：一句话说清事件，一个观察角度，一句轻度收尾。自然、口语、克制。

## 审查维度

### 1. 自然度（structure_score, 0-1）
- 是否像三段式自然过渡：事件概括 → 一个观察 → 轻度收尾
- 发现并列编号结构（第一、第二、第三）直接扣到 0.3 以下
- 发现"首先、其次、最后"过渡词直接扣到 0.3 以下
- 发现分点论述直接扣到 0.2
- 正文中出现 Markdown 格式符号（加粗星号、井号、下划线等）直接扣到 0.3 以下

### 2. 内容增量（content_score, 0-1）
- 不需要深度论证，但不能是"正确的废话"
- "这很正常""大家都不容易"这类空洞表达扣分
- 有一个具体的观察角度即可，不需要多个

### 3. 语气真实度（tone_score, 0-1）
- 像真人发朋友圈/即刻/小红书吗
- 有没有口语化表达
- 有没有轻微调侃但不攻击
- 读起来像聊天还是像念稿

### 4. 字数控制（length_score, 0-1）
- 180-250 字为满分区间
- 低于 120 字或超过 300 字扣分
- 低于 80 字或超过 350 字直接给 0.2

### 5. AI 味检测（ai_score, 0-1）
- "值得深思""这背后反映了""我们不得不承认" → 直接 0
- "时代的一粒灰""雪崩时" → 直接 0
- "未来已来""让我们拭目以待" → 直接 0
- "赋能""底层逻辑""颗粒度""抓手""闭环""范式" → 直接 0
- 发现 emoji 表情 → 扣到 0.5 以下
- 正文中出现加粗标记（两个星号包裹的文字）→ 扣到 0.3

## 输出格式

请严格按以下 JSON 格式输出，不要包含任何其他文字：

\`\`\`json
{
  "structure_score": 0.8,
  "content_score": 0.7,
  "tone_score": 0.6,
  "length_score": 0.9,
  "ai_score": 0.8,
  "overall_score": 0.74,
  "passed": true,
  "issues": ["结尾偏鸡汤", "中间观察太泛"],
  "suggestions": ["结尾改成轻度反问", "观察角度可以更具体"]
}
\`\`\`

评分标准：
- overall_score = structure_score * 0.2 + content_score * 0.25 + tone_score * 0.25 + length_score * 0.1 + ai_score * 0.2
- passed: overall_score >= 0.6 且 tone_score >= 0.5 且 ai_score >= 0.6
- 短评重点看语气和 AI 味`;

function buildCommentReviewPrompt(body: string): string {
  return `请审查以下热点短评的质量，严格按照评分标准打分。

## 待审查短评

${body}

请输出 JSON 审查结果：`;
}

export function buildCommentReviewSystemPrompt(body: string): ReviewPrompt {
  return {
    system: COMMENT_REVIEW_SYSTEM_PROMPT,
    user: buildCommentReviewPrompt(body),
  };
}

export interface CommentReviewResult {
  structure_score: number;
  content_score: number;
  tone_score: number;
  length_score: number;
  ai_score: number;
  overall_score: number;
  passed: boolean;
  issues: string[];
  suggestions: string[];
}

export function parseCommentReviewResult(raw: string): CommentReviewResult | null {
  try {
    const jsonMatch = raw.match(/```json\s*([\s\S]*?)\s*```/) || raw.match(/\{[\s\S]*\}/);
    if (!jsonMatch) return null;

    const jsonStr = jsonMatch[1] || jsonMatch[0];
    const data = JSON.parse(jsonStr);

    return {
      structure_score: data.structure_score ?? 0,
      content_score: data.content_score ?? 0,
      tone_score: data.tone_score ?? 0,
      length_score: data.length_score ?? 0,
      ai_score: data.ai_score ?? 0,
      overall_score: data.overall_score ?? 0,
      passed: data.passed ?? false,
      issues: data.issues ?? [],
      suggestions: data.suggestions ?? [],
    };
  } catch {
    return null;
  }
}

export function mapCommentReviewToReviewResult(cr: CommentReviewResult): ReviewResult {
  return {
    structure_score: cr.structure_score,
    content_score: cr.content_score,
    tone_score: cr.tone_score,
    opening_score: cr.length_score,
    ending_score: cr.ai_score,
    overall_score: cr.overall_score,
    passed: cr.passed,
    issues: cr.issues,
    suggestions: cr.suggestions,
  };
}

export function parseReviewResult(raw: string): ReviewResult | null {
  try {
    const jsonMatch = raw.match(/```json\s*([\s\S]*?)\s*```/) || raw.match(/\{[\s\S]*\}/);
    if (!jsonMatch) return null;

    const jsonStr = jsonMatch[1] || jsonMatch[0];
    const data = JSON.parse(jsonStr);

    return {
      structure_score: data.structure_score ?? 0,
      content_score: data.content_score ?? 0,
      tone_score: data.tone_score ?? 0,
      opening_score: data.opening_score ?? 0,
      ending_score: data.ending_score ?? 0,
      overall_score: data.overall_score ?? 0,
      passed: data.passed ?? false,
      issues: data.issues ?? [],
      suggestions: data.suggestions ?? [],
    };
  } catch {
    return null;
  }
}
